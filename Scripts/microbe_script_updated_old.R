...
######## color boxes for background plot
data_long_day %>%
  filter(name == "do_mg_l",
         time_point == "end",
         foundation_spp != "Ocean")%>%
  ggplot(aes(x = foundation_spp, y = removal_control, fill = mean_val))+
  geom_tile()+
  scale_fill_gradient(low ="#D3E3CA", high = "#4A5438" )+
  facet_wrap(~month)+
  theme_classic()
ggsave(here("Output","backgroundcolors.pdf"), width = 8, height = 8)
...
mean_rates %>%
  filter(name == "do_mg_l",
         ...
         foundation_spp != "Ocean")%>%
  ungroup()%>%
  select(foundation_spp, removal_control, mean_value, before_after)%>%
  pivot_wider(names_from = before_after,
              values_from = mean_value,values_fn = mean
                )%>%
  mutate(change =  After - Before,
         dummyx = 1,
         dummy = 1:4) 
  # ggplot(aes(x = dummyx, y = dummy, linewidth = change, group = foundation_spp))+
  # geom_line()+
  # facet_grid(~removal_control)
